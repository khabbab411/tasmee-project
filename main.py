...
async def handle_callback(update: Update, context):
    ...
    if callback_data == "menu_submit":
        ...
        if user_id in pending_user_actions and current_state == "awaiting_confirmation":
            ...
            reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("رد", callback_data=f"reply_{submission_id}") ]])

            if action_data["file_id"]:
                message_to_group = await context.bot.send_voice(chat_id=GROUP_ID, voice=action_data["file_id"], caption=caption_text, reply_markup=reply_markup)
            elif action_data["text_content"]:
                message_to_group = await context.bot.send_message(chat_id=GROUP_ID, text=f"{caption_text}\n\n{action_data['text_content']}", reply_markup=reply_markup)
        ...
        submission_id = int(callback_data.split("_")[1])
        
        conn = sqlite3.connect(DATABASE_NAME)
        c = conn.cursor()
        c.execute("SELECT s.user_id, s.name, sub.submission_type FROM submissions sub JOIN students s ON sub.user_id = s.user_id WHERE sub.submission_id=?", (submission_id,))
        row = c.fetchone()
        conn.close()
        
        if row:
            student_id, student_name, sub_type = row
            waiting_for_reply[teacher_id] = {
                "student_id": student_id,
                "student_name": student_name,
                "submission_id": submission_id,
                "submission_type": sub_type,
                "type": "text" # افتراضي نص، سيتم استقبال أي منهما
            }
            
            keyboard = [
                [InlineKeyboardButton("رد نصي ✍️", callback_data=f"set_reply_type_text_{submission_id}")],
                [InlineKeyboardButton("رد صوتي 🎙️", callback_data=f"set_reply_type_voice_{submission_id}")]
            ]
            await query.message.reply_text(f"جاري تحضير الرد على {student_name} ({sub_type}). اختر نوع الرد:", reply_markup=InlineKeyboardMarkup(keyboard))
        else:
            await query.message.reply_text("عذراً، لم يتم العثور على بيانات هذا الطلب.")

    elif callback_data.startswith("set_reply_type_"):
        parts = callback_data.split("_")
        r_type = parts[3]
        submission_id = int(parts[4])
        teacher_id = query.from_user.id
        
        if teacher_id in waiting_for_reply:
            waiting_for_reply[teacher_id]["type"] = r_type
            msg = "الرجاء كتابة الرد النصي الآن:" if r_type == "text" else "الرجاء تسجيل الرد الصوتي الآن:"
            await query.edit_message_text(msg)
        else:
            await query.edit_message_text("انتهت صلاحية الجلسة، يرجى الضغط على 'رد' مرة أخرى.")

async def report_command(update: Update, context):
    # السماح بالتقرير فقط في المجموعة المحددة
    if update.effective_chat.id != GROUP_ID:
        await update.message.reply_text("عذراً، هذا الأمر مخصص للمعلمات في المجموعة الخاصة فقط.")
        return
